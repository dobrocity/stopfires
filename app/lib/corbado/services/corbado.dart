...
import 'package:stopfires/corbado/exceptions.dart';
...
class CorbadoService {
  ...
  Future<AuthenticateRequestType> startLoginWithPasskey(
    ...
    try {
      ...
      return StartLoginResponse.fromJson(
        json,
      ).toPlatformType(conditional: conditional);
    } on FirebaseFunctionsException catch (e) {
      throw _convertFirebaseFunctionsException(e);
    } catch (e) {
      rethrow;
  ...
  Future<void> deleteUser(String firebaseToken) async {
    try {
      await _functions
          .httpsCallable(
            kDebugMode ? 'deleteUser' : 'ext-authentication-corbado-deleteUser',
            options: _functionOptions,
          )
