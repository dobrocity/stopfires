...
// Resolve service account path (relative paths in .env are resolved to absolute)
const serviceAccountPath = path.resolve(
  __dirname,
  process.env.CUSTOM_GOOGLE_APPLICATION_CREDENTIALS || ''
);
...
const baseUrl = `https://corbado.stopfires.org`;

if (!CORBADO_PROJECT_ID || !CORBADO_API_SECRET) {
  throw new Error(
    'Missing environment variables. You must set CORBADO_PROJECT_ID and CORBADO_API_SECRET'
  );
}

const corbadoService = new CorbadoService(
  baseUrl,
  CORBADO_PROJECT_ID,
  CORBADO_API_SECRET
);
...
const parseRequestMetadata = (
  ...
  let userAgent = callableRequest.rawRequest.headers['user-agent'] as
    | string
    | undefined;
  const remoteAddress = callableRequest.rawRequest.headers[
    'x-forwarded-for'
  ] as string | undefined;
...
export const startSignUpWithPasskey = onCall(
...
export const finishSignUpWithPasskey = onCall(
  async (callableRequest: CallableRequest) => {
    const params = callableRequest.data as PasskeyRegisterFinish;
    const metadata = parseRequestMetadata(callableRequest);

    try {
      const corbadoUser = await corbadoService.finishSignUpWithPasskey(
...
export const startLoginWithPasskey = onCall(
  async (callableRequest: CallableRequest) => {
    try {
      ...
      return await corbadoService.startLoginWithPasskey(
        params.username,
        metadata
      );
    } catch (e) {
...
export const finishLoginWithPasskey = onCall(
  async (callableRequest: CallableRequest) => {
    try {
      const params = callableRequest.data as PasskeyLoginFinish;
      const metadata = parseRequestMetadata(callableRequest);
      const corbadoUser = await corbadoService.finishLoginWithPasskey(
        params.signedChallenge,
        metadata
      );
...
export const startLoginWithEmailOTP = onCall(
  async (callableRequest: CallableRequest) => {
    try {
      const params = callableRequest.data as EmailOTPLoginStart;
      const metadata = parseRequestMetadata(callableRequest);
      return await corbadoService.startLoginWithEmailOTP(
        params.username,
        metadata
      );
    } catch (e) {
...
export const finishLoginWithEmailOTP = onCall(
  async (callableRequest: CallableRequest) => {
    try {
...
export const startPasskeyAppend = onCall(
  async (callableRequest: CallableRequest) => {
    try {
      ...
      const res = await corbadoService.startPasskeyAppend(
        verifiedToken.email,
        verifiedToken.email,
        metadata
      );
...
export const finishPasskeyAppend = onCall(
  async (callableRequest: CallableRequest): Promise<boolean> => {
    const params = callableRequest.data as PasskeyAppendFinish;
    const metadata = parseRequestMetadata(callableRequest);
    try {
      const verifiedToken = await verifyIdToken(params.firebaseToken);
      const corbadoUserId = await corbadoService.finishPasskeyAppend(
        params.signedChallenge,
        metadata
      );
...
const verifyIdTokenAndGetCorbadoUserId = async (
  token: string
): Promise<string> => {
...
const maybeUpdateClaims = async (
  firebaseUser: UserRecord,
  corbadoUserId: string
) => {
